  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-foreground">{t.admin.overview.title}</h1>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        />
        <StatCard
          title={t.admin.overview.newUsers}
          ...
          value={
            stats?.recentPayments
              ? t.admin.common.amountAed.replace(
                  "{amount}",
                  stats.recentPayments.reduce((s, p) => s + Number(p.price_aed), 0).toFixed(0)
                )
              : null
          }
          icon={CreditCard}
        ...
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="flex items-center gap-2 text-base">
              ...
              {t.admin.overview.todayAppointments}
            </CardTitle>
          </CardHeader>
          <CardContent>
            {loading ? (
              ...
              <p className="text-sm text-muted-foreground">
                {t.admin.overview.noAppointmentsToday}
              </p>
            ) : (
                ...
                {stats.recentAppointments.map((a) => (
                  ...
                  >
                    ...
                    <span className="flex-1 truncate px-2 text-muted-foreground capitalize">
                      {getConsultationTypeLabel(t, a.consultation_type)}
                    </span>
                    <span
        ...
        <Card>
          ...
          <CardContent>
            {loading ? (
              ...
              <p className="text-sm text-muted-foreground">{t.admin.overview.noPayments}</p>
            ) : (
              <ul className="flex flex-col gap-2">
                {stats.recentPayments.map((p) => (
                  ...
                  >
                    <span className="font-medium text-foreground">
                      {t.admin.common.amountAed.replace("{amount}", String(p.price_aed))}
                    </span>
